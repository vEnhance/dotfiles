...
call IMAP('.describe ', "\\begin{description}\<CR>\\ii[<++>] <++>\<CR>\\end{description}<++>\<ESC>k<<",'tex')
...
call IMAP('.casy ', "\\begin{center}\<CR>\\begin{asy}\<CR><++>\<CR>\\end{asy}\<CR>\\end{center}<++>", 'tex')
...
call IMAP('.begin ', "\\begin{<++>}\<CR><++>\<CR>\\end{<++>}<++>", 'tex') " BEST IDEA EVER
...
" TeX Customizations
let g:Tex_FoldedEnvironments+=',asy,usamts' " Folding of certain environments
