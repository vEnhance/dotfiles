...
with get_session() as session:
    ...
    recent_txn = [
        txn for txn in citi.transactions
        ...
        for row in reader:
            ...
            for txn in recent_txn:
                if abs(row_date - txn.date) <= timedelta(
                        days=1) and row_amount == txn.amount:
                ...
                if 'Pythonanywhere' in row_description and row_amount < 0:
                    ...
                    row_description = 'Autopay for ' + row_date.strftime(
                        '%b %Y')
                elif (row_description.startswith('Uber Eats') or
