...
with get_session() as session:
    for bank_account_name in ('TCS', 'TCC'):
        ...
        recent_txn = [
            txn for txn in bank.transactions
        ...
        for tr in soup.find_all('tr', class_='Data'):
            ...
            row_amount = (-to_dollars(str_amount[2:-1])
                          if str_amount.startswith('(') else to_dollars(
                              str_amount[1:]))
            ...
            for txn in recent_txn:
                if abs(row_date - txn.date) <= timedelta(
                        days=7) and row_amount == txn.amount:
                ...
                if row_description == 'ACH Deposit MASS INST' and row_amount > 0:
                    ...
                    account_name = 'I:MIT'
                elif row_description == 'ACH Withdrawal UTILITY MGMT SOL' and row_amount < 0:
                    row_description = 'Water bill for ' + row_date.strftime(
                        '%b %Y')
                    account_name = 'E:House:Util'
                elif row_description == 'ACH Withdrawal NATIONAL GRID NE' and row_amount < 0:
                    row_description = 'Gas bill for ' + (
                        row_date + timedelta(days=-28)).strftime('%b %Y')
                    account_name = 'E:House:Util'
                elif row_description == 'ACH Withdrawal EVERSOURCE' and row_amount < 0:
                    row_description = (
                        'Electricity bill for ' +
                        (row_date + timedelta(days=-14)).strftime('%b %Y'))
                    account_name = 'E:House:Util'
                elif row_description == 'Transfer Withdrawal' and row_amount == -1000 and row_date.day < 7:
                    row_description = 'Mortgage contribution to parents'
                    account_name = 'E:House:Mortg'
                elif row_description == 'Dividend Deposit Tiered Rate' and row_amount > 0:
                    row_description = 'APY Interest'
                    account_name = 'I:AntiFee'
                elif row_description == 'ACH Withdrawal CITI AUTOPAY' and row_amount < 0:
                    row_description = 'Autopay for ' + row_date.strftime(
                        '%b %Y')
                    account_name = 'L:Citi'
                elif row_description.startswith(
                        'ACH Deposit Twitch') and row_amount > 0:
                    row_description = 'Twitch streamer payout'
                    account_name = 'I:Twitch'
                elif row_description.startswith(
                        'ACH Withdrawal TD BANK') and row_amount < 0:
                    row_description = 'TD payment'
                    account_name = 'L:TD'
                elif row_description == 'ACH Deposit STRIPE' and bank_account_name == 'TCC':
