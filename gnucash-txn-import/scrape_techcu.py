	...
	for bank_account_name in ('TCS', 'TCC'):
		...
		for tr in soup.find_all('tr', class_='Data'):
			...
			row_amount = (
				...
				if str_amount.startswith('(') else to_dollars(str_amount[1:])
			)
			...
			for txn in recent_txn:
				if abs(row_date - txn.date) <= timedelta(days=7) and row_amount == txn.amount:
				...
				if row_description == 'ACH Deposit MASS INST' and row_amount > 0:
					...
					row_description = 'Gas bill for ' + (row_date + timedelta(days=-28)).strftime('%b %Y')
					account_name = 'E:House:Util'
				elif row_description == 'ACH Withdrawal EVERSOURCE' and row_amount < 0:
					row_description = (
						'Electricity bill for ' + (row_date + timedelta(days=-14)).strftime('%b %Y')
					)
					account_name = 'E:House:Util'
