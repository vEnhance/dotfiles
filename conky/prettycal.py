...
tasks_json = subprocess.run(task_cmd_args,
...
for t in tasks_dicts:
    ...
    all_items[calitem.when.date() if calitem.type != Type.NOW else None].append(
        calitem)
...
table[HEADER_Y_FIRST][
    0] = r'${font Exo 2:size=%d:bold}${color 55ff99}Upcoming Events' % (
        FONT_SIZE + 3)
table[HEADER_Y_SECOND][
    0] = r'${font Exo 2:size=%d:bold}${color ff5599}Other Tasks' % (FONT_SIZE +
                                                                    3)
