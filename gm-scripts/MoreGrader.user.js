  ...
  AoPS.Contests.Views.ContestGraderEntry.extend({
    ...
    setTplVars: function () {
      ...
      for (var i = 0; i < evals.length; i++) {
        if (
          Number(evals[i].grader_id) === Number(AoPS.session.user_id) &&
          (evals[i].evaluation_state === "done" ||
            evals[i].evaluation_state === "conflict")
        ) {
          i_am_done = true;
        }
        if (
      ...
      _.extend(this.vars, {
        ...
        evals: this.model.get("evals").map(
          _.bind(function (t) {
            var i = {
              ...
              graded:
                "done" === t.evaluation_state ||
                "conflict" === t.evaluation_state,
              grader_display_name: t.grader_id
                ? this.model
                    .get("contest")
                    .get("graders")
                    .get(t.grader_id)
                    .get("display_name")
                : "N/A",
              score: _.isNull(t.score_override)
                ? Math.round(t.score)
                : Math.round(t.score_override),
            ...
            return (
              (i.buttons = {
                grade:
                  "new" === t.evaluation_state &&
                  Number(t.grader_id) !== Number(AoPS.session.user_id),
              ...
              (i.is_already_claimed = !i.mine && "new" !== t.evaluation_state),
              i
            );
