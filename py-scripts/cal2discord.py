...
calendars: List[Tuple[str, Any]] = [
	(_['name'], Calendar.from_ical(requests.get(_['url']).text)) for _ in options['calendars']
]

events: List[Tuple[str, Event]] = []
for (calname, calendar) in calendars:
	for component in recurring_ical_events.of(calendar).between(interval_start, interval_end):
...
calendar_lines[row + 2] = (
	calendar_lines[row + 2][:3 * k] + '💚' + calendar_lines[row + 2][3 * k + 2:]
)
calendar_text = '\n'.join(calendar_lines)
embed = DiscordEmbed(
	title=options.get('title', 'Calendar'),
	description=(
		f"Generated on {now.strftime('%A, %B %d, %H:%M')}\n"
		f"Time zone: {tz.zone}\n"
		f"```\n{calendar_text}\n```"
	),
