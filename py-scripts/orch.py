...
ids = json.loads(
    Path('~/Sync/otis-evil-data/evil.json').expanduser().read_text())
choices = '\n'.join(k + '\t' + v for k, v in ids.items())

try:
    chosen = subprocess.check_output(
        args=[
            'fzf', '-e', '--tabstop', '12', '-d', r'\t', '--preview',
            'python -m von show {2}', '--preview-window', 'down'
        ],
        input=choices,
        text=True,
    )
...
resp = requests.post(OTIS_API_URL,
                     json={
                         'action': 'get_hints',
                         'puid': puid,
...
if type(result) == dict:
    if 'new_hints' in result:
        new_hint_dicts = [
            d for d in result['new_hints']
            if ((type(d.get('number')) == int) and
                (d.get('keywords') is not None) and
