...
parser.add_argument(
    ...
    help=
    'Print output to this file (only works with one filename), pass with no argument for stdout.'
)
parser.add_argument(
    '-f',
    '--full',
...
parser.add_argument(
    ...
    type=str,
    help=
    'Name of competition for section headers (defaults to reading from filename).'
)
...
def main(tsv_file, outfile, name):
    ...
    print(r"\text{Top %d}&%d" % (args.a, scores_total[-args.a]),
          r"\\",
          file=outfile)
    ...
    scores_by_pr: List[List[int]] = [[] for i in range(NUM_PROBLEMS)
                                    ]  # scores by problem
    for line in tsv_lines:
        if not line.strip:
            continue
        data = line.split('\t')
        studentscores = [
            int(x) if (x in '01234567' and x) else 0 for x in data[:-1]
        ]
        assert sum(studentscores) == int(
            data[-1]), f"Total doesn't match: {studentscores} != {data[-1]}"
    ...
    print(''.join([
        ...
        (width, i + 1) for i in range(NUM_PROBLEMS)
    ]),
          end=' ',
          file=outfile)
    print(r"\\ \hline", file=outfile)
    ...
    print(r" & ".join([
        get_bottom_cell(sum(int(x >= 5) for x in scores_by_pr[i]),
                        percent=False) for i in range(NUM_PROBLEMS)
    ]),
          file=outfile)
    print(r"\\", file=outfile)
    # Compute %(solve)
    print(r"{}^{\%}5+ &", file=outfile)
    print(r" & ".join([
        get_bottom_cell(sum(int(x >= 5)
                            for x in scores_by_pr[i]), percent=True)
        for i in range(NUM_PROBLEMS)
    ]),
          file=outfile)
    print(r"\\ \hline", file=outfile)
    ...
    if args.full is True:
        ...
        print(r"\begin{longtable}{r|" + "r" * NUM_PROBLEMS + "|r}",
              file=outfile)
        print("Rank & " + " & ".join(f"P{i+1}" for i in range(NUM_PROBLEMS)),
              file=outfile)
        print(r"& $\Sigma$ \\ \hline \endhead", file=outfile)
        for scores in scores_raw:
            total = ssum(scores)
            print(r"{\footnotesize\sffamily %3d.} &    " % ranks[total],
                  end='',
                  file=outfile)
            print(' & '.join([str(_) if _ is not None else '-' for _ in scores
                             ]),
                  end='',
                  file=outfile)
...
if __name__ == "__main__":
    ...
    assert len(files) == 1 or (
        ...
        args.name is None), "can't write multiple inputs to a single output"
