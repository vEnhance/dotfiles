...
parser.add_argument(
	'-a', action='store', default=3, type=int, help='First stat is top A; by default A = 3.'
)
parser.add_argument(
	'-b', action='store', default=12, type=int, help='First stat is top B; by default B = 12.'
)
...
def main(tsv_file, outfile, name):
	...
	for s in range(0, 8):  # scores 0-7
		print(
			str(s) + " & " +
			"\n\t& ".join([get_table_cell(s, scores_by_pr[i].count(s)) for i in range(NUM_PROBLEMS)]),
			end=' ',
			file=outfile
		)
		print(r"\\", file=outfile)
	...
	def printMP(S0, start, stop):
		...
		while i >= stop:
			...
			print(
				r"%2d  %3d  %3d  %.2f%%" % (i, scores_total.count(i), S, float(S) * 100.0 / N),
				file=outfile
			)
			i -= 1
			ranks[i] = S + 1
	...
	if args.full is True:
		...
		print("Rank & " + " & ".join(f"P{i+1}" for i in range(NUM_PROBLEMS)), file=outfile)
		print(r"& $\Sigma$ \\ \hline \endhead", file=outfile)
		for scores in scores_raw:
			total = ssum(scores)
			print(r"{\footnotesize\sffamily %3d.} &    " % ranks[total], end='', file=outfile)
			print(
				' & '.join([str(_) if _ is not None else '-' for _ in scores]), end='', file=outfile
			)
