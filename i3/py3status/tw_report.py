...
Configuration parameters:
	...
	report: name of report to use (default 'undone')
...
class Py3status:
	...
	def post_config_hook(self):
		...
		if self.filter:
			self.taskwarrior_command = f"task {self.filter} export {self.report}"
		else:
			self.taskwarrior_command = f"task export {self.report}"
